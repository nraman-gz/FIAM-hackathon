...
if __name__ == "__main__":
    ...
    while (starting + pd.DateOffset(years=11 + counter)) <= pd.to_datetime(
        ...
        # Use 30 epochs for first iteration, 15 for subsequent iterations
        max_epochs = 15 if (counter > 0) else 30
        ...
        print(f"Iteration {counter}: train_max = {train_max}, val_max = {val_max}")
        print(f"  Predictions before merge: {len(test_pred_df)}")
        print(f"  Prediction time_idx range: {test_pred_df['time_idx'].min()} - {test_pred_df['time_idx'].max()}")
        print(f"  Test data time_idx range: {test_data['time_idx'].min()} - {test_data['time_idx'].max()}")
        print(f"  Test data shape: {test_data.shape}")
        print(f"  Unique stock IDs in predictions: {test_pred_df['id'].nunique()}")
        print(f"  Unique stock IDs in test_data: {test_data['id'].nunique()}")
        ...
        print(f"  After merge: {len(test_pred_df)} predictions")
        ...
        if len(test_pred_df) > 0:
            ...
            print(f"  WARNING: No predictions after merge for iteration {counter}!")
            continue
